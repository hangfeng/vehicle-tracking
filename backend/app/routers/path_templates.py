...
async def create_template(
    ...
    tmpl = PathTemplate(name=body.name, factory_id=fid)
    db.add(tmpl)
    await db.flush()
    for s in body.steps:
...
async def get_template(
    ...
    query = select(PathTemplate).where(PathTemplate.id == tmpl_id).options(selectinload(PathTemplate.steps))
    if user.role != UserRole.group_admin:
        query = query.where(PathTemplate.factory_id == user.factory_id)
...
async def update_template(
    ...
    query = select(PathTemplate).where(PathTemplate.id == tmpl_id)
    if user.role != UserRole.group_admin:
        query = query.where(PathTemplate.factory_id == user.factory_id)
    result = await db.execute(query)
    tmpl = result.scalar_one_or_none()
